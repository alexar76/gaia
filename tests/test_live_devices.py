...
OPENAQ = {
    "results": [
        ...
        {"parameter": {"name": "pm10"}, "latest": {"value": 18.0}},
    ]
}
...
def test_firms_stratified_spreads_across_grid_cells(tmp_path):
    from gaia.devices.live_open import FirmsFireHotspot

    lines = ["latitude,longitude,bright_ti4,confidence"]
    # bright band at 30°N
    for i in range(40):
        lines.append(f"30.0,{10.0 + i * 0.1:.1f},400.0,h")
    # dimmer band at 25°N — lost under global top-N
    for i in range(40):
        lines.append(f"25.0,{10.0 + i * 0.1:.1f},350.0,h")
    csv = "\n".join(lines) + "\n"
    dev = FirmsFireHotspot("firms-fire-01", SimClock(), key_dir=tmp_path)
    bbox = (0.0, 20.0, 30.0, 35.0)
    bright_only, _, _, _ = dev.collect_hotspots(csv, limit=20, bbox=bbox)
    stratified, _, _, _ = dev.collect_hotspots(
        csv, limit=20, bbox=bbox, stratified=True, grid_deg=5.0
    )
    assert all(h["latitude"] == pytest.approx(30.0) for h in bright_only)
    assert any(h["latitude"] == pytest.approx(25.0) for h in stratified)
    assert any(h["latitude"] == pytest.approx(30.0) for h in stratified)
...
def test_build_live_fleet_registers_tier_a_b(tmp_path, monkeypatch):
    ...
    assert {"nws-01", "osm-01", "sta-01", "om-wx-01", "om-aq-01",
            ...
            "adsb-lol-01", "hms-smoke-01", "usgs-wq-01", "noaa-dart-01",
            "nexrad-status-01"} <= ids
    assert {"cams-ottawa", "cams-berlin", "cams-delhi", "cams-tokyo",
            "cams-nairobi", "cams-sao-paulo"} <= ids
    assert {"radnet-birmingham", "radnet-washington", "radnet-los-angeles"} <= ids
    assert {"dart-21414", "dart-32401", "dart-55012"} <= ids
    assert sum(device_id.startswith("radnet-") for device_id in ids) == 140
    assert sum(device_id.startswith("dart-") or device_id == "noaa-dart-01" for device_id in ids) == 43
    ...
    assert "sc-stuttgart" in ids
    assert "sc-munich" in ids
    by_id = {d["device_id"]: d for d in fleet.status()["devices"]}
    assert "sensor.community" in by_id["sc-01"]["source"].lower()
    assert "sensor.community" in by_id["sc-stuttgart"]["source"].lower()
