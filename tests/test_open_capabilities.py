...
def test_air_capability_sc_accepts_buyer_lat_lon(live_env, monkeypatch):
    """gaia.air.read@v1 + sc-01 must rebuild the Sensor.Community area= URL."""
    from gaia.devices.live_p0 import SensorCommunityAir

    seen: list[str] = []
    payload = [
        {
            "location": {"latitude": "48.13510", "longitude": "11.58200"},
            "sensordatavalues": [
                {"value_type": "P2", "value": "11.0"},
                {"value_type": "P1", "value": "19.0"},
            ],
        }
    ]
    ...
    sc = live_env.fleet.get("sc-01")
    assert isinstance(sc, SensorCommunityAir)
    ...
    assert data["ok"] is True
    vals = data["output"]["reading"]["values"]
    assert vals["pm2_5_ugm3"] == pytest.approx(11.0)
    assert any("area=48.13510,11.58200" in u for u in seen), seen
    # Coordinate must not stick after the invoke.
    assert "area=52.52000,13.41000" in sc.url

    spec = build_spec(live_env, public_url="http://gaia.test")
    air = next(c for c in spec.capabilities if c.capability_id == "gaia.air.read@v1")
    assert "sc-01" in air.description
    assert "latitude/longitude" in air.description
    assert "latitude" in (air.input_schema or {}).get("properties", {})
