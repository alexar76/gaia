...
def test_sensor_community_set_coordinate_rebuilds_area_url(tmp_path):
    clock = SimClock(realtime=True)
    dev = SensorCommunityAir(
        "sc-01", clock, latitude=52.52, longitude=13.41, radius_km=1.0, key_dir=str(tmp_path),
    )
    assert "area=52.52000,13.41000,1.00" in dev.url
    with dev.query_gate:
        dev.set_coordinate(48.1351, 11.5820)
        assert "area=48.13510,11.58200,1.00" in dev.url
        assert "area=52.52000" not in dev.url
        dev.clear_coordinate()
    assert "area=52.52000,13.41000,1.00" in dev.url


def test_sensor_community_empty_area_offline(tmp_path):
    clock = SimClock(realtime=True)
    dev = SensorCommunityAir("sc-01", clock, key_dir=str(tmp_path))
    with pytest.raises(DeviceOffline, match="no PM"):
        dev.map([])
