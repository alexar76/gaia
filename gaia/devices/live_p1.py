...
def register_p1_relays(fleet: Any, clock: SimClock, *, key_dir: str) -> int:
    ...
    if enabled("GAIA_AURN_ENABLED", "1"):
        for device_id, site_code, _lat, _lon, _place in (
            ("aurn-01", "MY1", 51.5225, -0.1546, "London Marylebone Road"),
            ...
            try:
                fleet.add(
                    DefraAurn(
                        device_id, clock,
                        site_code=site_code,
                        site=f"live-air-{device_id}",
                        key_dir=key_dir,
                    )
                )
                n += 1
            except ValueError as exc:
                log.warning("Defra AURN %s skipped: %s", device_id, exc)
    ...
    if enabled("GAIA_UHSLC_ENABLED", "1"):
        # IDs verified against UHSLC Fast Delivery ERDDAP (labels match station_name).
        for device_id, uhslc_id, _lat, _lon, _place in (
            ("uhslc-01", "57", 21.3067, -157.8667, "Honolulu"),
            ...
            try:
                fleet.add(
                    UhslcTide(
                        device_id, clock,
                        uhslc_id=uhslc_id,
                        site=f"live-tide-{device_id}",
                        key_dir=key_dir,
                    )
                )
                n += 1
            except ValueError as exc:
                log.warning("UHSLC %s skipped: %s", device_id, exc)
