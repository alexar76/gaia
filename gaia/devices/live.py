...
class OpenAQLocation(LiveDevice):
    ...
    def __init__(self, device_id: str, clock: SimClock, *,
        ...
        # /sensors embeds parameter + latest; /latest alone only returns sensorsId.
        self.url = f"https://api.openaq.org/v3/locations/{location_id}/sensors"

    def map(self, payload: Any) -> dict[str, float | None]:
        # v3 sensors: {"results":[{"parameter":{"name":"pm25"},"latest":{"value":…}}, …]}
        # Older /latest shape (tests): {"results":[{"value":…,"parameter":{"name":"pm25"}}, …]}
...
def build_live_fleet(clock: SimClock, key_dir: str = "data/devices") -> Fleet:
    ...
    from gaia.devices.live_p4 import register_p4_relays

    n_p4 = register_p4_relays(fleet, clock, key_dir=key_dir)
    if n_p4:
        log.info("Registered %s P4 environmental LIVE relay(s)", n_p4)

    from gaia.devices.live_p5 import register_p5_relays

    n_p5 = register_p5_relays(fleet, clock, key_dir=key_dir)
    if n_p5:
        log.info("Registered %s P5 LIVE relay(s)", n_p5)

    from gaia.devices.live_p6 import register_p6_relays

    n_p6 = register_p6_relays(fleet, clock, key_dir=key_dir)
    if n_p6:
        log.info("Registered %s P6 LIVE relay(s)", n_p6)

    from gaia.devices.live_p7 import register_p7_relays

    n_p7 = register_p7_relays(fleet, clock, key_dir=key_dir)
    if n_p7:
        log.info("Registered %s P7 LIVE relay(s)", n_p7)

    from gaia.devices.live_p8 import register_p8_relays

    n_p8 = register_p8_relays(fleet, clock, key_dir=key_dir)
    if n_p8:
        log.info("Registered %s P8 LIVE relay(s)", n_p8)

    from gaia.devices.live_p9 import register_p9_relays

    n_p9 = register_p9_relays(fleet, clock, key_dir=key_dir)
    if n_p9:
        log.info("Registered %s P9 LIVE relay(s)", n_p9)

    from gaia.devices.live_p10 import register_p10_relays

    n_p10 = register_p10_relays(fleet, clock, key_dir=key_dir)
    if n_p10:
        log.info("Registered %s P10 LIVE relay(s)", n_p10)

    from gaia.devices.live_p11 import register_p11_relays

    n_p11 = register_p11_relays(fleet, clock, key_dir=key_dir)
    if n_p11:
        log.info("Registered %s P11 LIVE relay(s)", n_p11)

    from gaia.devices.live_p12_jma import register_p12_jma

    n_p12_jma = register_p12_jma(fleet, clock, key_dir=key_dir)
    if n_p12_jma:
        log.info("Registered %s P12 JMA LIVE relay(s)", n_p12_jma)

    _p12_slices: tuple[tuple[str, str], ...] = (
    ...
    n_p12 = 0
    for _mod_name, _fn_name in _p12_slices:
        try:
            _mod = __import__(_mod_name, fromlist=[_fn_name])
            _register = getattr(_mod, _fn_name)
        except ImportError:
            continue
        try:
            _added = _register(fleet, clock, key_dir=key_dir)
        except Exception as exc:
            log.warning("P12 slice %s failed: %s", _mod_name, exc)
            continue
        n_p12 += int(_added or 0)
    try:
        from gaia.devices.live_p12 import _register_p12_gfm

        n_p12 += _register_p12_gfm(fleet, clock, key_dir=key_dir)
    except ImportError:
        pass
    if n_p12:
        log.info("Registered %s P12 LIVE relay(s)", n_p12)
