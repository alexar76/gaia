...
class NWSStation(LiveDevice):
    ...
    headers = {"User-Agent": "gaia-oracle-demo (contact@example.com)"}
...
def build_live_fleet(clock: SimClock, key_dir: str = "data/devices") -> Fleet:
    ...
    box_id = os.environ.get("GAIA_OSM_BOX_ID", "").strip() or "example-box-id"
    sta_base = os.environ.get("GAIA_STA_BASE_URL", "").strip() or _STA_DEFAULT_BASE
    sta_ds = os.environ.get("GAIA_STA_DATASTREAM", "").strip() or "example-datastream-id"
