...
# Units for GAIA fields a live device may relay (mirrors plausibility.PHYSICS).
...
class OpenMeteoAirQuality(LiveDevice):
    ...
    def __init__(self, device_id: str, clock: SimClock, *,
        ...
        self.url = (
            ...
            f"?latitude={latitude:.5f}&longitude={longitude:.5f}"
            "&current=pm2_5,pm10,carbon_dioxide"
        )
...
def build_live_fleet(clock: SimClock, key_dir: str = "data/devices") -> Fleet:
    ...
    # Own-edge ADS-B / AIS feeders (opt-in; offline until ingest push).
