...
#: Distinct strings the per-parse pool holds. `satellite`, `instrument`, `daynight`, `version` take a
#: dozen values across an ENTIRE FIRMS day and `observed_at` at most 1440, so this cap only matters
#: for a malformed feed putting high-cardinality junk in those columns.
_STRING_POOL_MAX = 20_000
...
def _clamp_grid_deg(raw: Any, default: float = 5.0) -> float:
    try:
        g = float(raw) if raw is not None else default
    except (TypeError, ValueError):
        g = default
    return max(0.5, min(g, 30.0))
...
def _stratified_pick(
    ...
    *,
    ...
    """Top brightest per global grid cell, then fill remaining slots globally."""
    if not scored or cap <= 0:
        return []
    grid_deg = _clamp_grid_deg(grid_deg)
    buckets: dict[tuple[int, int], list[tuple[float, dict[str, Any]]]] = {}
    for rank, row in scored:
        lat = float(row["latitude"])
        lon = float(row["longitude"])
        li = int((lat + 90.0) / grid_deg)
        wi = int((lon + 180.0) / grid_deg)
        buckets.setdefault((li, wi), []).append((rank, row))
    occupied = len(buckets)
    if occupied == 0:
        return []
    per_cell = max(1, cap // occupied)
    out: list[dict[str, Any]] = []
    seen: set[tuple[float, float]] = set()
    for key in sorted(buckets.keys()):
        bucket = sorted(buckets[key], key=lambda t: t[0], reverse=True)
        for _, row in bucket[:per_cell]:
            dedupe = (round(float(row["latitude"]), 4), round(float(row["longitude"]), 4))
            if dedupe in seen:
                continue
            seen.add(dedupe)
            out.append(row)
...
class FirmsFireHotspot(LiveDevice):
    ...
    def __init__(self, device_id: str, clock: SimClock, *, map_key: str = "", **kw):
        ...
        self._query_stratified: bool = False
        self._query_grid_deg: float = 5.0
    ...
    def set_query(
        ...
        *,
        ...
        with self._lock:
            ...
            self._query_stratified = bool(stratified)
            self._query_grid_deg = _clamp_grid_deg(grid_deg)
    ...
    def clear_query(self) -> None:
        with self._lock:
            ...
            self._query_stratified = False
            self._query_grid_deg = 5.0
    ...
    def collect_hotspots(
        ...
        *,
        ...
        """
        # One list, not two. This used to build a FILTERED COPY of every line in the day's CSV on top
        # of the list `splitlines()` already returns — ~250 000 strings held twice for a global day,
        # at peak, right before the rows themselves are built. Blank lines are skipped in the loop
        # below instead.
        ...
        # One shared string pool for the whole parse pass. Every row carried its OWN freshly
        # allocated copies of the same handful of values — `value[:80]` and that f-string mint a new
        # object per row — and a global fire day is hundreds of thousands of rows.
        #
        # That is what took GAIA's backend to 3.04 GB in four days and the oracle host into swap: a
        # heap census found two HotspotSession lists of 140 781 rows each, and the cost was in the
        # DUPLICATED STRINGS, not the number of fires. Pooling changes no value — the strings are
        # equal, they are simply shared — so not one hotspot and not one field is lost. Dropping rows
        # would have been the wrong trade: a global day really does carry that many fires, and
        # retaining a fraction would make the product lie about the world.
        pool: dict[str, str] = {}
        ...
        def provenance(cols: list[str]) -> dict[str, Any]:
            ...
            for key in ("satellite", "instrument", "daynight", "version"):
                value = cell(cols, key)
                if value:
                    out[key] = shared(value[:80])
            if date:
                out["acq_date"] = shared(date[:16])
            if raw_time:
                out["acq_time_utc"] = shared(digits)
            if observed_at:
                out["observed_at"] = shared(observed_at)
        ...
        best_rank = float("-inf")
        for ln in islice(lines, 1, _CSV_SCAN_CAP + 1):
            if not ln.strip():
                continue
            cols = ln.split(",")
        ...
        if stratified and bbox is not None:
            rows = _stratified_pick(scored, cap, grid_deg=grid_deg)
        else:
            rows = [row for _, row in scored[:cap]]
        truncated = in_scope > len(rows)
        return rows, truncated, matched_global, best_global
    ...
    def read(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            stratified = self._query_stratified
            grid_deg = self._query_grid_deg
        text = self._fetch_text(self.url)
