...
class VirtualDevice:
    ...
    # Licence-required credit. A CC BY / attributed source is only redistributable
    # if the credit travels WITH the artifact — the buyer holds the reading, not
    # our /wot directory. Set to a source_id in gaia.source_policy; the reading
    # then carries the registry's attribution string. Empty = nothing is owed
    # (simulators, U.S. public domain), and the key is omitted rather than faked.
    policy_id: str = ""

    @property
    def attribution(self) -> str:
        """Attribution text this device's licence obliges a buyer to carry."""
        if not self.policy_id:
            return ""
        from gaia.source_policy import APPROVED_SOURCES

        policy = APPROVED_SOURCES.get(self.policy_id)
        return policy.attribution if policy else ""
    ...
    #: A relay must bring an observation. Simulators always do; a live relay whose upstream
    #: answered with nothing but nulls (or only the coordinate it was asked about) has no
    #: reading to sign, and signing it sold an empty result as a success.
    requires_values = False
    _NON_OBSERVATION_FIELDS = frozenset({"latitude", "longitude", "lat", "lon"})

    def read(self) -> dict[str, Any]:
        """One attested reading: values + device metadata + device signature."""
        honest = self.sample()
        if self.requires_values and not (set(honest) - self._NON_OBSERVATION_FIELDS):
            raise DeviceOffline(f"{self.device_id}: upstream answered with no values")
