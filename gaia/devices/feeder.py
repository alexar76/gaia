...
third-party aggregators as the provenance source.
"""
...
class FeederDevice(LiveDevice):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.kind = kind
        if kind == "adsb":
            self.fields = dict(_ADS_B_FIELDS)
        elif kind == "ais":
            self.fields = dict(_AIS_FIELDS)
        else:
            self.fields = dict(_IOT_FIELDS)
        ...
        if provenance:
            ...
            self.source = (
                "operator edge feeder (own IoT / Tasmota / TTN / SenML push; "
                "not a third-party aggregator)"
            )
