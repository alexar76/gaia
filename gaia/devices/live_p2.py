...
ECCC_HYDRO_MESH: tuple[tuple[str, str, float, float, str], ...] = (
    ("eccc-hydro-01", "02HC003", 43.7006, -79.5208, "Humber River at Weston"),
    ("eccc-hydro-ottawa-01", "02OA016", 45.4280, -73.6700, "St. Lawrence at LaSalle"),
    ("eccc-hydro-credit-01", "02HB001", 43.8167, -80.0167, "Credit River near Cataract"),
    ("eccc-hydro-calgary-01", "05BH004", 51.0450, -114.0719, "Bow River at Calgary"),
    ("eccc-hydro-fraser-01", "08MF005", 49.3833, -121.4500, "Fraser River at Hope"),
    ("eccc-hydro-princealbert-01", "05GG001", 53.2000, -105.7500, "N. Saskatchewan at Prince Albert"),
    ("eccc-hydro-stillwater-01", "01EO001", 45.1500, -62.0667, "St. Marys at Stillwater"),
    ("eccc-hydro-liard-01", "10ED001", 60.2333, -123.4667, "Liard River at Fort Liard"),
)

FMI_MESH: tuple[tuple[str, str, float, float, str], ...] = (
    ("fmi-01", "Helsinki", 60.1699, 24.9384, "Helsinki"),
    ("fmi-tampere-01", "Tampere", 61.4978, 23.7610, "Tampere"),
    ("fmi-turku-01", "Turku", 60.4518, 22.2666, "Turku"),
    ("fmi-oulu-01", "Oulu", 65.0121, 25.4651, "Oulu"),
    ("fmi-rovaniemi-01", "Rovaniemi", 66.5039, 25.7294, "Rovaniemi"),
)
...
def register_p2_relays(fleet: Any, clock: SimClock, *, key_dir: str) -> int:
    ...
    if enabled("GAIA_ECCC_HYDRO_ENABLED", "1"):
        for device_id, station, _lat, _lon, _place in ECCC_HYDRO_MESH:
            try:
                fleet.add(
                    EcccHydrometric(
                        device_id,
                ...
                log.warning("ECCC hydrometric %s skipped: %s", device_id, exc)
    if enabled("GAIA_FMI_ENABLED", "1"):
        for device_id, place, _lat, _lon, _label in FMI_MESH:
            try:
                fleet.add(
                    FmiWeather(
                        device_id,
                ...
                log.warning("FMI weather %s skipped: %s", device_id, exc)
