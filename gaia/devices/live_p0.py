...
def signed_cluster_read(
    ...
    *,
    ...
    reading_meta: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Attest headline plus the complete cluster for ATLAS fan-out.

    ``structured_keys`` is intentionally opt-in: most event pins stay compact,
    while geometry-bearing feeds can retain JSON objects/lists without flattening
    them to a 500-character string. The attestation canonical binds ``hotspots``.
    """
    if not hotspots:
    ...
    for row in hotspots:
        ...
        for key in structured_keys:
            ...
            if isinstance(val, (dict, list)):
                # Round-trip rejects custom objects and detaches the signed value.
                try:
                    item[key] = json.loads(json.dumps(val, allow_nan=False))
    ...
    if isinstance(reading_meta, dict):
        for key, val in reading_meta.items():
...
class SensorCommunityAir(LiveDevice):
    ...
    Default pin is operator-anchored (Berlin or mesh city). Buyers may pass
    ``latitude`` / ``longitude`` on ``gaia.air.read@v1`` to query any area where
    crowd sensors exist — empty area → offline (honesty: SC density ≠ global).
    """
    ...
    def __init__(
        ...
        **kw,
        ...
        if not (-90.0 <= float(latitude) <= 90.0 and -180.0 <= float(longitude) <= 180.0):
            raise ValueError("Sensor.Community lat/lon out of range")
        self.latitude = float(latitude)
        self.longitude = float(longitude)
        self._default_coordinate = (self.latitude, self.longitude)
        self.radius_km = max(0.1, min(float(radius_km), 25.0))
        self._default_radius_km = self.radius_km
    ...
    def _coordinate_changed(self) -> None:
        self.url = (
            ...
            f"area={self.latitude:.5f},{self.longitude:.5f},{self.radius_km:.2f}"
        )
    ...
    def clear_coordinate(self) -> None:
        ...
        self.radius_km = self._default_radius_km
        self._coordinate_changed()
...
def register_p0_relays(fleet: Any, clock: SimClock, *, key_dir: str) -> int:
    ...
    if enabled("GAIA_SC_ENABLED", "1"):
        ...
        n += 1
        # City mesh anchors (SC-dense regions). Buyers still pass any lat/lon on
        # gaia.air.read@v1 — these pins are map density, not every SDS011 node.
        if enabled("GAIA_SC_MESH_ENABLED", "1"):
            from gaia.devices.sc_mesh import SC_MESH_CITIES
            ...
            for city in SC_MESH_CITIES:
                ...
                fleet.add(
                    SensorCommunityAir(
                        ...
                        radius_km=float(city.get("radius_km") or 5.0),
                        site=f"live-air-sc-{slug}",
                        key_dir=key_dir,
                    )
                )
                n += 1
