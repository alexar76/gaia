...
class VerifierService:
    ...
    def verify(self, raw_input: Any, min_verify_score: float | None = None) -> dict[str, Any]:
        ...
        if verdict.verified:
            # A caller's own bar decides the caller's verdict, but it must NOT move the
            # shared escrow high-water: min_verify_score=0.0 makes any parseable reading
            # "verified", and advancing on that lets an unauthenticated caller mark every
            # pending settlement stale (each later reading comes back "replay rejected",
            # which the hub settles as a real provider failure). Mutating shared state
            # requires clearing the LOCAL threshold — the operator's bar, not the caller's.
            advance = verdict.score >= float(self.verifier.threshold)
            fresh = self._check_and_advance_freshness(reading, advance=advance)
    ...
    def _check_and_advance_freshness(self, reading: dict[str, Any], *, advance: bool = True):
        ...
        ``advance=False`` still rejects a replay but leaves the high-water alone — for
        a verdict that only passed because the caller lowered the bar below the local
        threshold, so it may not settle anything and may not stale anything either."""
