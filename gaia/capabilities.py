...
def _p4_geo_handler(runtime: GatewayRuntime, default_device: str):
    ...
    def handler(data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(device, UsgsWaterQuality) or not all(bbox_fields):
            point_types = (
                OpenMeteoAirQuality, CamsAirComposition, CopernicusSoilWaterIndex, NasaImergPrecipitation,
            ...
            if not isinstance(device, point_types) or not has_lat:
                return runtime.read(device_id)
            with device.query_gate:
                device.set_coordinate(float(raw["latitude"]), float(raw["longitude"]))
                try:
                    return runtime.read(device_id)
                finally:
...
def build_spec(runtime: GatewayRuntime, public_url: str | None = None) -> OracleSpec:
    ...
    caps = [
        ...
        Capability(
            ...
            description="Live relay or sim air-quality (PM2.5/PM10/AQI/CO2/VOC), Ed25519-attested. "
                        "om-aq-01 accepts any latitude/longitude through the commercial or "
                        "self-hosted Open-Meteo licence gate; other device_ids remain fixed.",
            handler=_p4_geo_handler(runtime, "aq-01"),
