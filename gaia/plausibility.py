...
PHYSICS: dict[str, FieldPhysics] = {
    ...
    # Live relays beyond indoor sensors — event / grid / coastal series.
    # z_check off: half-hour grid steps and earthquake events are not OU noise.
    "carbon_intensity_gco2_kwh": FieldPhysics(0.0, 1500.0, 80.0, None, 5.0, z_check=False),
    "magnitude": FieldPhysics(0.0, 10.0, 10.0, None, 0.1, z_check=False, stuck_check=False),
    "depth_km": FieldPhysics(-5.0, 800.0, 50.0, None, 1.0, z_check=False),
    "latitude": FieldPhysics(-90.0, 90.0, 180.0, None, 0.01, z_check=False, stuck_check=False),
    "longitude": FieldPhysics(-180.0, 180.0, 360.0, None, 0.01, z_check=False, stuck_check=False),
    "water_level_m": FieldPhysics(-5.0, 20.0, 0.5, 0.15, 0.02, stuck_check=True),
    # Rivers / marine (USGS NWIS, NOAA NDBC, Open-Meteo Marine)
    "discharge_m3s": FieldPhysics(0.0, 200_000.0, 500.0, None, 0.1, z_check=False),
    # USGS gage datum is typically < 50 m; ECCC hydrometric LEVEL can be geodetic.
    "gage_height_m": FieldPhysics(-5.0, 200.0, 0.8, 0.25, 0.02, stuck_check=True),
    "wave_height_m": FieldPhysics(0.0, 30.0, 2.0, None, 0.05, z_check=False),
    "sst_c": FieldPhysics(-2.0, 40.0, 1.5, 0.5, 0.1, stuck_check=True),
    # Free-to-commercialize event / feeder series (not OU noise).
    "brightness_k": FieldPhysics(200.0, 600.0, 50.0, None, 1.0, z_check=False, stuck_check=False),
    "confidence": FieldPhysics(0.0, 100.0, 100.0, None, 1.0, z_check=False, stuck_check=False),
    "cpm": FieldPhysics(0.0, 1_000_000.0, 500.0, None, 1.0, z_check=False),
    "severity_score": FieldPhysics(0.0, 100.0, 100.0, None, 1.0, z_check=False, stuck_check=False),
    "radius_km": FieldPhysics(0.0, 5_000.0, 100.0, None, 1.0, z_check=False),
    "altitude_m": FieldPhysics(-500.0, 20_000.0, 500.0, None, 10.0, z_check=False),
    "speed_mps": FieldPhysics(0.0, 400.0, 50.0, None, 1.0, z_check=False),
    # Maritime bound, not the AIS encoding range: 102.2 kn is the "or higher"
    # bucket, which in practice only ever means a broken encoder. Relays drop
    # those rather than widening the envelope to admit them.
    "sog_knots": FieldPhysics(0.0, 60.0, 5.0, None, 0.1, z_check=False),
    "cog_deg": FieldPhysics(0.0, 360.0, 360.0, None, 1.0, z_check=False, stuck_check=False),
    # NHC intensity is knots; keep separate from weather wind_mps (75 m/s cap).
    "intensity_kn": FieldPhysics(0.0, 200.0, 20.0, None, 1.0, z_check=False, stuck_check=False),
    # P0/P1 extras — event scores and in-situ geophysics (not OU noise).
    "kp_index": FieldPhysics(0.0, 9.0, 9.0, None, 0.1, z_check=False, stuck_check=False),
    "aurora_pct": FieldPhysics(0.0, 100.0, 100.0, None, 1.0, z_check=False, stuck_check=False),
    "energy_j": FieldPhysics(0.0, 1e-3, 1e-6, None, 0.0, z_check=False, stuck_check=False),
    "energy_fj": FieldPhysics(0.0, 1e9, 1e6, None, 0.0, z_check=False, stuck_check=False),
    "field_nt": FieldPhysics(20_000.0, 70_000.0, 500.0, None, 10.0, z_check=False),
    "salinity_psu": FieldPhysics(0.0, 45.0, 1.0, None, 0.01, z_check=False),
    "pressure_dbar": FieldPhysics(0.0, 11_000.0, 50.0, None, 0.1, z_check=False),
    "demand_mw": FieldPhysics(0.0, 1_000_000.0, 50_000.0, None, 1.0, z_check=False),
    # UK Daily Air Quality Index: 1 (low) … 10 (very high).
    "air_quality_index": FieldPhysics(
        1.0, 10.0, 9.0, None, 1.0, z_check=False, stuck_check=False
    ),
    "us_aqi": FieldPhysics(0.0, 500.0, 500.0, None, 1.0, z_check=False, stuck_check=False),
    "european_aqi": FieldPhysics(0.0, 500.0, 500.0, None, 1.0, z_check=False, stuck_check=False),
    "water_temperature_c": FieldPhysics(-2.0, 45.0, 2.0, 1.0, 0.1, stuck_check=True),
    "ph": FieldPhysics(0.0, 14.0, 1.0, 0.5, 0.05, stuck_check=True),
    "dissolved_oxygen_mg_l": FieldPhysics(0.0, 25.0, 2.0, 1.0, 0.05, stuck_check=True),
    "specific_conductance_us_cm": FieldPhysics(0.0, 100_000.0, 500.0, None, 1.0, z_check=False),
    "water_column_height_m": FieldPhysics(0.0, 12_000.0, 5.0, None, 0.01, z_check=False),
    "measurement_type": FieldPhysics(1.0, 3.0, 2.0, None, 1.0, z_check=False),
    "precipitation_mm_h": FieldPhysics(0.0, 500.0, 100.0, None, 0.1, z_check=False),
    "radar_latency_s": FieldPhysics(0.0, 86_400.0, 60.0, None, 0.01, z_check=False),
    "reflectivity_calibration_db": FieldPhysics(-20.0, 20.0, 5.0, None, 0.01, z_check=False),
    "transmitter_power_w": FieldPhysics(0.0, 5_000.0, 500.0, None, 1.0, z_check=False),
    "aerosol_optical_depth": FieldPhysics(0.0, 10.0, 1.0, None, 0.01, z_check=False),
    "dust_ugm3": FieldPhysics(0.0, 100_000.0, 500.0, None, 1.0, z_check=False),
    "alder_pollen_grains_m3": FieldPhysics(0.0, 100_000.0, 500.0, None, 1.0, z_check=False),
    "birch_pollen_grains_m3": FieldPhysics(0.0, 100_000.0, 500.0, None, 1.0, z_check=False),
    "grass_pollen_grains_m3": FieldPhysics(0.0, 100_000.0, 500.0, None, 1.0, z_check=False),
    "dose_equivalent_nsv_h": FieldPhysics(0.0, 10_000_000.0, 1_000.0, None, 1.0, z_check=False),
    "gamma_count_total_cpm": FieldPhysics(0.0, 10_000_000.0, 10_000.0, None, 1.0, z_check=False),
    "soil_water_index_pct": FieldPhysics(0.0, 100.0, 20.0, None, 0.1, z_check=False),
    "solar_irradiation_kwh_m2_day": FieldPhysics(0.0, 20.0, 10.0, None, 0.01, z_check=False),
    "clear_sky_irradiation_kwh_m2_day": FieldPhysics(0.0, 20.0, 10.0, None, 0.01, z_check=False),
    "solar_observation_yyyymmdd": FieldPhysics(19810101.0, 21001231.0, 400.0, None, 1.0, z_check=False),
    "snow_depth_cm": FieldPhysics(0.0, 2_000.0, 300.0, None, 0.01, z_check=False),
    "snow_water_equivalent_cm": FieldPhysics(0.0, 1_000.0, 100.0, None, 0.001, z_check=False),
    "snow_observation_yyyymmdd": FieldPhysics(20030930.0, 21001231.0, 400.0, None, 1.0, z_check=False),
    "sea_ice_concentration_pct": FieldPhysics(0.0, 100.0, 50.0, None, 0.1, z_check=False),
    "sea_ice_observation_yyyymmdd": FieldPhysics(19781026.0, 21001231.0, 400.0, None, 1.0, z_check=False),
    "land_surface_temperature_c": FieldPhysics(-100.0, 100.0, 30.0, None, 0.01, z_check=False),
    "land_surface_temperature_uncertainty_k": FieldPhysics(0.0, 50.0, 10.0, None, 0.01, z_check=False),
    # P5
    "dewpoint_c": FieldPhysics(-80.0, 40.0, 2.0, 1.0, 0.1, stuck_check=True),
    "speed_kmh": FieldPhysics(0.0, 350.0, 40.0, None, 0.01, z_check=False),
    "drought_pct": FieldPhysics(0.0, 100.0, 20.0, None, 0.01, z_check=False),
    "solar_wind_kms": FieldPhysics(200.0, 1200.0, 100.0, None, 0.01, z_check=False),
    "bt_nt": FieldPhysics(0.0, 100.0, 10.0, None, 0.01, z_check=False),
    "xray_flux": FieldPhysics(0.0, 1e-3, 1e-4, None, 0.0, z_check=False),
    # P6 reservoirs and forecast UV.
    "pool_elev_m": FieldPhysics(-100.0, 5_000.0, 10.0, None, 0.01, z_check=False),
    "storage_m3": FieldPhysics(0.0, 1e14, 1e8, None, 1.0, z_check=False),
    "uv_index": FieldPhysics(0.0, 30.0, 10.0, None, 0.1, z_check=False),
    "rainfall_mm": FieldPhysics(0.0, 1_000.0, 100.0, None, 0.01, z_check=False),
}
...
class PlausibilityVerifier:
    ...
    def check(self, reading: dict[str, Any], attestation: dict[str, Any] | None = None,
        ...
        # Round FIRST, then compare. The rounded number is the one that leaves this
        # process (envelope `verify_score` / `delivery_verdict.score`), and the hub's
        # escrow re-checks `score >= threshold` against it: deciding `verified` on the
        # unrounded value let the two disagree by up to 5e-5, and an envelope that
        # says fulfils=true with a score under the bar is rejected as
        # `delivery_verdict_inconsistent` — a forced refund for an honest reading.
        score = round(self._score(checks), 4)
        ...
        return Verdict(score >= threshold, score, checks, summary)
    ...
    def _prior_history(self, device_id: str, reading: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # Same site only: a coordinate-queried relay reads many places under one id, and
        # its Berlin history says nothing about whether a Delhi reading is plausible.
        site = reading.get("site")
        return [h for h in self.fleet.history(device_id)
                if h.get("seq", 0) < seq and h.get("site") == site]
